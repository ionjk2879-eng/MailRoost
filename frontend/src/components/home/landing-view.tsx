...
export function LandingView({ onConnected }: LandingViewProps) {
  return (
    <div className="flex min-h-svh flex-col">
      ...
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col items-center justify-center gap-12 px-6 py-16">
        {/* Hero */}
        <div className="flex flex-col items-center gap-6 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            <Button render={<a href={gmailLoginUrl} />} size="sm" className="gap-2">
              Gmail 연결
            </Button>
            <ConnectNaverDialog
              label="네이버 메일 연결"
        ...
        <div className="w-full">
          ...
          <div className="overflow-hidden rounded-xl border shadow-sm">
            {/* Account chips */}
            <div className="bg-muted/40 flex items-center gap-2 border-b px-4 py-2.5 flex-wrap">
            ...
            {PREVIEW_MAILS.map((mail) => {
              ...
              return (
                ...
                >
                  <span
                    className={`mt-1.5 size-2 shrink-0 rounded-full ${account?.color ?? "bg-gray-400"}`}
                  />
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      <span
                        className={`text-xs ${!mail.isRead ? "font-semibold" : "text-muted-foreground"}`}
                      >
                        {mail.fromName}
                      </span>
