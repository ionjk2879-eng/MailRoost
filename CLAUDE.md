...
사용자에게 답할 때는 항상 한국어 존댓말(~습니다/~요 체)을 사용한다. 설명/분석/제안 내용이 영어로 먼저 떠올랐거나 영어 자료를 참고했더라도, 사용자에게 보여주는 문장은 반드시 한국어로 번역해서 전달한다 (코드 식별자, 파일 경로, 에러 메시지 원문 인용은 예외). 단, 커밋 메시지·PR 본문은 기존 관례대로 영어를 유지한다.
